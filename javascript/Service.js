      ...
      function updateDisplay(searchTerm) {
        ...
        unavailableService.forEach((unavailable) => {
          unavailable.addEventListener("click", function () {
            ...
            Swal.fire({
              title: "Make " + serviceName + " Available?",
              text: "You won't be able to revert this.",
              icon: "warning",
              showCancelButton: true,
              confirmButtonColor: "#73E977",
        ...
        addServices.addEventListener("click", () => {
          formContainer.classList.remove("hidden");
          addServicesForm.classList.remove("hidden");
          const checkboxes = addServicesForm.querySelector("#checkboxes");
          const templateCheckbox = document.querySelector("#templateCheckbox");
          const templateCheckboxLoader = document.querySelector(
            "#templateCheckboxLoader"
          );
          ...
          data.forEach((obj) => {
            ...
            clone.querySelector("#serviceOriginalName").textContent =
              obj.serviceName;
            ...
            // Modify the input value and name
            const input = clone.querySelector('input[type="checkbox"]');
            input.value = obj.id;
            input.id = obj.id;
            input.name = obj.serviceName.toLowerCase().replace(" ", "-"); //palitan mo na lang yung name ng checkboxes

            checkboxes.appendChild(clone);
          });

          var cancelAdd = addServicesForm.querySelector("#cancelAdd");

          cancelAdd.addEventListener("click", function () {
            formContainer.classList.add("hidden");
            addServicesForm.classList.add("hidden");

            var preview = addServicesForm.querySelector("#preview");
            var serviceDescriptionAdd = addServicesForm.querySelector(
              "#serviceDescription"
            );
            var serviceNameAdd =
              addServicesForm.querySelector("#serviceNameAdd");

            preview.src = "../images/placeholder.png";
            serviceDescriptionAdd.value = "";
            serviceNameAdd.value = "";
          });

          var saveService = addServicesForm.querySelector("#saveService");
          saveService.addEventListener("click", function () {
            var dropArea = addServicesForm.querySelector("#dropArea");
            var fileInput = addServicesForm.querySelector("#fileInput");
            var serviceDescriptionAdd = addServicesForm.querySelector(
              "#serviceDescription"
            );
            var serviceNameAdd =
            ...
            if (fileInput.value === "") {
              dropArea.style.borderColor = "var(--red)";
              return;
            } else if (fileInput.value !== "") {
              dropArea.style.borderColor = "";
            }

            if (serviceDescriptionAdd.value === "") {
              serviceDescriptionAdd.style.borderColor = "var(--red)";
              return;
            } else if (serviceDescriptionAdd.value !== "") {
              serviceDescriptionAdd.style.borderColor = "";
            }

            if (serviceNameAdd.value === "") {
              serviceNameAdd.style.borderColor = "var(--red)";
              return;
            } else if (serviceNameAdd.value !== "") {
              serviceNameAdd.style.borderColor = "";
            } else {
              dropArea.style.borderColor = "";
              serviceDescriptionAdd.style.borderColor = "";
              serviceNameAdd.style.borderColor = "";
